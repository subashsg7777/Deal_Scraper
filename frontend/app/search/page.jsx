...
import { ChevronRight, Gamepad2, Search as SearchIcon, Sparkles } from 'lucide-react'
...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const response = await fetch(`/api/search?term=${encodeURIComponent(trimmedQuery)}`)
  ...
  const gameName = (game) => game.name ?? game.gameName ?? game.title ?? 'Unknown Game'
